 ...
 *  分隔符跟随前缀首个分隔符（C:/docs → /，C:\docs → \；混合前缀 C:/Users\me 跟 /），尾部斜杠去重。 */
export const joinPath = (dir: string, name: string): string => {
  if (!dir) return name;
  const d = dir.replace(/[\\/]+$/, '');
  const i = d.search(/[\\/]/);
  return `${d}${i >= 0 ? d[i] : '\\'}${name}`;
};
...
/** 目录句柄一次授权长期记忆（v2.0.2 ensureDir）：IndexedDB 记忆句柄 granted 直接用零弹窗；
 *  'prompt' 且处在用户手势链内（openDoc/saveDocAs 均由点击触发）则 requestPermission 一次；
 *  都没有或明示拒绝才 showDirectoryPicker。 */
async function ensureDir(): Promise<FileSystemDirectoryHandle | null> {
  if (!hasFS()) return null;
  const saved = await idbGet<FileSystemDirectoryHandle>('dir');
  if (saved) {
    if (!saved.queryPermission || !saved.requestPermission) return saved; // 旧内核无权限 API 视为已授权（同 permit 约定）
    const d: Perm = { mode: 'readwrite' };
    if ((await saved.queryPermission(d)) === 'granted') return saved;
    if ((await saved.requestPermission(d)) === 'granted') return saved;
  }
  if (!window.showDirectoryPicker) return null;
  try {
    const dir = await window.showDirectoryPicker({ id: 'md2prompt-dir' });
    return (await permit(dir)) ? dir : null;
  } catch { return null; }
}
...
/** 拖放打开（v2.0.2）：FS 句柄优先走全链路（可回写 + 目录记忆），无句柄落 files 降级（内存保留、保存走下载）。 */
export async function openDropped(dt: DataTransfer | null): Promise<DocFile | null> {
  if (!dt) return null;
  const generation = ++targetGeneration;
  ...
  if (!/\.(md|markdown|jsonl|ndjson|xml)$/i.test(name)) return null; // 与 ACCEPT 同一白名单
  if (hasFS() && item) {
    try {
      const h = await (
      ...
      if (h && h.kind === 'file') {
        ...
        if (!(await permit(fh)) || generation !== targetGeneration) return null;
        ...
        dirHandle = dir;
        void idbPut('file', fh);
        return { name: fh.name, kind: kindOf(fh.name), text, mtime: f.lastModified };
      }
    } catch { /* 句柄不可用（合成事件/旧内核）：落 files 降级 */ }
  }
  if (!file) return null;
  const text = await file.text();
  if (generation !== targetGeneration) return null;
  setDoc(null, file.name);
  return { name: file.name, kind: kindOf(file.name), text, mtime: file.lastModified };
}
...
export async function saveDocAs(t: string): Promise<boolean> { // 显式保存（新文档首次保存 / fallback 下载）
  ...
  try {
    ...
    if (generation !== targetGeneration) return false;
    const dir = await ensureDir(); // save-as 后补请目录授权（记忆句柄优先，v2.0.2），否则 Prompt.md 无法落盘
    if (generation !== targetGeneration) return false;
    setDoc(h, h.name);
    dirHandle = dir;
    void idbPut('file', h);
