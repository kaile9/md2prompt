...
/** 恢复三选「忽略」：本次 load 全程不再覆写既有 Prompt.md（B2——旧版只取消当次排程，首次编辑即破功）。 */
const promptSuppressed = new WeakSet<DocState>();
/** 日记构建/落盘失败监听（A-3：文档本体保存有指示，日记失败此前静默）。 */
const promptErrorListeners = new Set<() => void>();
export function onPromptError(cb: () => void): () => void {
  promptErrorListeners.add(cb);
  return () => {
    promptErrorListeners.delete(cb);
  };
}
...
function commit(persistPrompt = true): void {
  ...
  if (persistPrompt && !promptSuppressed.has(d)) schedulePrompt(d);
  notify();
}
...
export async function buildPrompt(d: DocState, copy = false): Promise<string> {
  ...
  if (d.file.kind !== 'jsonl') // JSONL 记录原子单位永远全文形；md/xml 够省走 patch 形
...
async function writePromptDebounced(d: DocState, target: fs.PromptTarget, seq: number): Promise<void> {
  try {
    // 哈希/渲染失败只丢本次 Prompt 落盘，doc 保存不受影响；下轮 commit 重试——但告知（A-3）
    const text = await buildPrompt(d);
    if (promptSeq.get(d) === seq) void fs.writePrompt(text, target);
  } catch {
    promptErrorListeners.forEach((f) => f());
  }
}
  ...
  dispatch(a: Action): void {
    ...
    switch (a.type) {
      ...
      case 'suppressPrompt': {
        ...
        promptSuppressed.add(doc); // B2：本次 load 全程不再覆写（后续编辑 commit 也跳过 schedulePrompt）
